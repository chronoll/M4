...
本記事では、User-Agentの基本・歴史・活用例・課題・新しいアプローチまでを、実際に手を動かして確認しながら解説します。

== User-Agentとは

==== リクエストヘッダとしてのUser-Agent

「User agent」という語句自体は「ユーザーの代理として動作するソフトウェア」という一般的な意味を持ちますが、本記事ではHTTPリクエストヘッダーとしての @<code>{User-Agent} を取り扱います。リクエストを送信したアプリケーションやオペレーティングシステム（OS）、ベンダー、バージョンなどを、サーバーやネットワーク側で識別するための特性文字列として用いられます。
...
User-Agentヘッダーが最初に登場したのは、最初期のHTTP仕様である@<b>{RFC 1945（HTTP/1.0）}です。この仕様では、User-Agentリクエストヘッダーは「そのリクエストを発生させたユーザーエージェントに関する情報を含む」ものとして定義されています。
...
現行のHTTPセマンティクスを定義するRFC 9110でも、User-Agentヘッダーは「リクエストを生成したユーザーエージェントソフトウェアについての情報を含むヘッダー」として引き続き定義されており、その立ち位置が変わることなくHTTP仕様の中で長年にわたり使われ続けてきました。
...
この末尾の文字列から、iPhoneのSafariからのアクセスであることがわかります。Google Analyticsではこうした情報をもとに、「ユーザー環境の詳細」レポートでブラウザやOS、デバイスカテゴリごとのセッション数やコンバージョン率を確認できます。普段何気なく見ているこれらのレポートも、裏側ではUser-Agent文字列の解析によって成り立っているのです。

@<href>{https://support.google.com/analytics/answer/12002752?hl==ja}
...
@<href>{https://humanwhocodes.com/blog/2010/01/12/history-of-the-user-agent-string/, History of the browser user-agent string} ではこれを「嘘の連鎖」という皮肉を含めながら経緯の詳細が語られているため、ぜひご一読ください。
...
@<href>{https://www.netassist.ne.jp/techblog/34866/, 「User-Agentを基盤とした悪意的アクセス判断の限界と対応戦略」}では、「User-Agentのスプーフィングは既に攻撃の基本要素になっている」とも表現されています。
...
Chromeはこの削減を2021年から7つのフェーズに分けて段階的に進め、2023年5月のChrome 113で完了しました。他のブラウザも同じ方向に動いています。Safariは実はChromeより早い2017年にUA文字列の凍結を試みたパイオニアで、現在はmacOSバージョンやCPUアーキテクチャを固定値にしています。Firefoxも同様に、OSバージョンやマイナーバージョンの凍結を段階的に進めています。
...
ただし、現時点でUA-CHを実装しているのはChromiumベースのブラウザ（ChromeやEdge）のみです。Firefoxを開発するMozillaはUA-CHの仕様に対して否定的な立場を表明しており、Safariも未対応です。そのため開発者は、ブラウザによってUA文字列とUA-CHを使い分ける必要があり、すぐにすべての環境で恩恵を受けられる状況ではありません。

====== UA-CHの確認方法

ChromeやEdgeブラウザのDevToolsコンソールで @<code>{navigator.userAgentData} を実行することで、UA-CHで提供される情報を確認できます。
...
先ほど述べた通り、これらの機能はFirefoxやSafariでは未対応です。試しにSafariの開発者ツールで出力を試みても、undefinedとなっています。
...
実際に、2024年にはAnthropicのClaudeBotやPerplexityのボットがrobots.txtの制限を無視してクロールしていたとして問題になったことがあります。User-Agent文字列を正直に名乗っていても、robots.txtのルールを遵守するかどうかは各ベンダーの実装次第です。

さらに新しい課題もあります。OpenAIのOperator（AIエージェントが実際にブラウザを操作するサービス）は、通常のChromeと同じUser-Agent文字列を使用しています。つまり、サーバーサイドからはAIエージェントのアクセスと人間のアクセスを、User-Agentだけでは区別できません。これは第4章で扱った「偽装」の問題とはまた別の次元で、User-Agentによる識別の限界を示しています。
