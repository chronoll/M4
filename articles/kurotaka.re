...
curl -s -o mobile.html -A "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac
OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148
Safari/604.1"https://www.google.com
...
==== 余談：ClaudeCodeにUser-Agent偽装を勧められた

最近のことです。ClaudeCodeにPrime Videoのリンクからタイトルやサムネイル画像を自動取得したいと相談したところ、「oEmbedやAPIではなくHTMLスクレイピングであり、User-Agentの偽装が必要です。実装を進めてよろしいですか？」とカジュアルに提案されました。あまりに自然だったので、いつも通り許可してしまうところでした。

いわゆるvibe codingの流行もあって、AIの提案をそのまま受け入れる場面だったり、ユーザーは増えていると思います。しかし、ノーチェックで任せてしまうと、意図せず利用規約に違反していたり、セキュリティ上のリスクを負ったり、今回場合では濫用するとIPアドレスごとブラックリスト入りしていたり……ということが普通に起こりえます。AIがあまりに当たり前の顔で提案してくるので、特に疑問を持たずに実装してしまっている人も一定数いるのではないかと思うと、なかなか怖いものがあります。

ちなみにこの話を社内で共有したところ、「AIはウェブ上の無数のコードや情報を公式にも非公式にも学習して生まれた存在。その出自からして、スクレイピングはある種DNA的に刻まれたプロセスであり、AIは自分を形作った行為をただ再現しているだけ、と言えるかもしれないですね、笑」というコメントが返ってきました。深い...

//image[claudecode][ClaudeCodeとの対話画面]{
一応リスクに触れてくれてはいるが、これを見逃さないバイブコーダーはどれだけいるだろうか？
//}
